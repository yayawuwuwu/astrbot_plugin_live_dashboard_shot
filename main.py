...
class LiveDashboardShot(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.config = config
        logger.info("✅ Live-Dashboard 截图插件已加载")
    ...
    async def _take_shot(self, event: AstrMessageEvent):
        try:
            ...
            yield event.plain_result(
                "截图功能依赖缺失，请先执行 pip install playwright Pillow && playwright install chromium"
            )
            return

        dashboard_url = self.config.get("dashboard_url", "http://172.17.0.1:7283")
        device_names_str = self.config.get(
            "device_names", "芽芽的拯救者y7000\n芽芽的一加ace5至尊版"
        )
            ...
            async with async_playwright() as p:
                ...
                if device_names:
                    ...
                    await page.wait_for_selector(
                        f"text={first_device}", timeout=10000
                    )
